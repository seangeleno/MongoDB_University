...
$unSet does the opposite and removes
...
db.movieDetails.updateOne({title: "The Martian"},
                          { $unSet: {poster: "http://ia.media-imdb.com/images/M/MV5BMTc2MTQ3MDA1Nl5BMl5BanBnXkFtZTgwODA3OTI4NjE@._V1_SX300.jpg"} })
```
$min and $max allow us to take in values with Comparison

###Updates are used to keep data current

####Tired of using your fingers to count how much you want to increase a data field by?
####$inc saves the day!

Using $set is an error prone approach.

Updates are used to correct errors and, over time, keep our data current. For movie data, much of what's there is static: directors, authors and the like. Other content such as reviews and ratings will need to be updated as users take action. We could use $set for this purpose, but that's an error prone approach. It's too easy to do the arithmetic incorrectly. Instead, we have a number of operators that support numeric updates of data: $min, $max, $inc, $mul. The following is an example using $inc to update reviews.
...
So, it adds it for you! no arithmetic required, how cool is that?
...
```
###Here we use updateOne to update an entire array
would normally think to use updateMany

####$each modifier is typical of updating arrays
####If you don't use $each then the entire array we specify in our call to $push will be added as a single element in that arrays
```javascript
db.movieDetails.updateOne({title: "The Martian"},
                          {$push: { reviews:
                                    { $each: [
...
Could do this, but it's probably the wrong semantics, because UNRATED and not having a rating are distinct
...
```
 Better to do this, which eliminates
...
db.movieDetails.updateOne(
    ...
    {upsert: true});
```
Rather than have two separate collections you just want one
replaceOne as with update commands takes filter and does a wholesale document search
So, in this example you can replace a less detailed summary with an entire collection that has was specifically
created for movie details
```javascript
...
##Here we switch to the db.reviews
